...
export default function Index({ employees_all }: Props) {
  ...
  const recalculateServiceTenure = () => {
    setLoading(true);
    router.post('/service-tenure/recalculate', {}, {
      onSuccess: () => {
        toast.success('Service tenure calculations refreshed successfully!');
        setLoading(false);
      },
      onError: (errors) => {
        console.error('Recalculation error:', errors);
        toast.error('Failed to refresh service tenure calculations');
        setLoading(false);
      },
      onFinish: () => {
        setLoading(false);
            ...
            <div className="m-3 no-scrollbar">
              <Card className="border-main bg-background drop-shadow-lg dark:bg-backgrounds">
                <CardHeader>
                  ...
                  <CardDescription>List of Service Tenure</CardDescription>
                </CardHeader>
                <CardContent>
                  {/* Replace with your data */}
                  <DataTable
                    columns={serviceTenureColumns(
                      setIsViewOpen,
                      setViewEmployee,
                      setIsModalOpen,
                      setEditModalOpen,
                    ...
                    recalculateFunction={recalculateServiceTenure}
